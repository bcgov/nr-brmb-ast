...
public class ProductiveUnitCodeEndpointsTest extends JerseyTest {
    ...
    protected Application configure() {
        ...
        config.register(ProductiveUnitCodeEndpointsImpl.class);
    ...
    public void testGetAllProductiveUnitCodes() throws Exception {
        Response response = target("/productiveUnitCodes").request().get();
        assertEquals(200, response.getStatus());

        String jsonString = response.readEntity(String.class);
        JSONObject jsonObject = new JSONObject(jsonString);

        assertEquals("ProductiveUnitCodeListRsrc", jsonObject.getString("@type"));

        JSONArray productiveUnitCodeList = jsonObject.getJSONArray("productiveUnitCodeList");
        JSONObject productiveUnitCode = productiveUnitCodeList.getJSONObject(0);
        assertEquals("100", productiveUnitCode.getString("code"));
        assertEquals("Alpaca", productiveUnitCode.getString("description"));
    }
}
