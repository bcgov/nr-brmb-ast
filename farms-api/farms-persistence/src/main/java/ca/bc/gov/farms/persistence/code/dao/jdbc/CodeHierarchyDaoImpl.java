package ca.bc.gov.farms.persistence.code.dao.jdbc;
...
public class CodeHierarchyDaoImpl extends BaseDao implements CodeHierarchyDao {
    ...
    @Override
    public CodeHierarchyDto fetch(CodeHierarchyConfig codeHierarchyConfig, LocalDate effectiveAsOfDate)
            throws DaoException {
    ...
    private List<HierarchyDto> select(Connection conn, CodeHierarchyConfig codeHierarchyConfig,
            LocalDate effectiveAsOfDate) throws DaoException {
        ...
        if (fetchSql == null || fetchSql.trim().length() == 0) {
            fetchSql = "select " + codeHierarchyConfig.getSkeyColumnName() + " SKEY, "
                    + codeHierarchyConfig.getUpperCodeColumnName() + " UPPER_CODE, "
                    + codeHierarchyConfig.getLowerCodeColumnName()
                    + " LOWER_CODE, ESTABLISHED_DATE, EXPIRY_DATE, WHO_CREATED, WHEN_CREATED, WHO_UPDATED, WHEN_UPDATED from "
    ...
    private void insert(Connection conn, CodeHierarchyConfig codeHierarchyConfig, HierarchyDto dto,
        ...
        if (insertSql == null || insertSql.trim().length() == 0) {
            ...
            insertSql = "insert into " + codeHierarchyConfig.getCodeHierarchyTableName() + " ("
                    ...
                    + "WHEN_UPDATED"
                    + ") values ("
                    + (codeHierarchyConfig.getSkeySequenceName() == null ? "sys_guid(),"
                            : codeHierarchyConfig.getSkeySequenceName() + ".nextval,")
                    + "?,"
                    + "?,"
        ...
        try (PreparedStatement st = conn.prepareStatement(insertSql)) {
            ...
            if (updateCount == 0) {
                throw new DaoException("Failed to insert code: " + codeHierarchyConfig.getCodeHierarchyTableName() + "."
                        + dto.getUpperCode() + "-" + dto.getLowerCode());
            }
    ...
    private void update(Connection conn, CodeHierarchyConfig codeHierarchyConfig, HierarchyDto dto,
            String currentUserId) throws DaoException {
        logger.debug("<update " + codeHierarchyConfig);
        ...
        if (updateSql == null || updateSql.trim().length() == 0) {
            ...
            updateSql = "update " + codeHierarchyConfig.getCodeHierarchyTableName() + " set "
                    ...
                    + ","
                    + "WHO_UPDATED = ?,"
                    + (Boolean.TRUE.equals(codeHierarchyConfig.getUseRevisionCount())
        ...
        try (PreparedStatement st = conn.prepareStatement(updateSql)) {
            ...
            int updateCount = st.executeUpdate();
            if (updateCount == 0) {
                throw new DaoException("Failed to update code: " + codeHierarchyConfig.getCodeHierarchyTableName() + "."
                        + dto.getUpperCode() + "-" + dto.getLowerCode());
            }
    ...
    private static void expire(Connection conn, CodeHierarchyConfig codeHierarchyConfig, HierarchyDto dto,
            String currentUserId) throws DaoException {
        logger.debug("<update " + codeHierarchyConfig);
