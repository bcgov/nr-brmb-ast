...
public class CodeTableDaoImpl extends BaseDao implements CodeTableDao {
        ...
        try (PreparedStatement st = conn.prepareStatement(insertSql)) {
            ...
            if (updateCount == 0) {
                throw new DaoException(
                        "Failed to isnert code: " + codeTableConfig.getCodeTableName() + "." + dto.getCode());
            }
    ...
    protected void update(Connection conn, FarmsCodeTableConfig codeTableConfig, CodeDto dto, String currentUserId)
        ...
        try (PreparedStatement st = conn.prepareStatement(updateSql)) {
            ...
            if (updateCount == 0) {
                throw new DaoException(
                        "Failed to update code: " + codeTableConfig.getCodeTableName() + "." + dto.getCode());
            }
    ...
    protected void expire(Connection conn, FarmsCodeTableConfig codeTableConfig, CodeDto dto, String currentUserId)
        ...
        if (expireSql == null || expireSql.trim().length() == 0) {
            expireSql = "UPDATE " + codeTableConfig.getCodeTableName() + " SET EXPIRY_DATE = CURRENT_DATE, "
                    + (Boolean.TRUE.equals(codeTableConfig.getUseRevisionCount())
                            ? "REVISION_COUNT = REVISION_COUNT + 1, "
                            : "")
        ...
        try (PreparedStatement st = conn.prepareStatement(expireSql)) {
            ...
            if (updateCount == 0) {
                throw new DaoException(
                        "Failed to expire code: " + codeTableConfig.getCodeTableName() + "." + dto.getCode());
            }
