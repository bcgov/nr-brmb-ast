    ...
    <td valign="top" align="right">
      <table class="numeric">
         <tr>
          ...
          <c:if test="${scenario.lateParticipant and form.hasEnhancedBenefits}">
            <th><fmt:message key="BC.Enhanced.Late.Enrolment.Benefit.Reduction"/></th>
          </c:if>
        ...
        <c:forEach var="curScenario" items="${scenario.combinedFarm.scenarios}">
          ...
          <tr>
            ...
            <c:if test="${scenario.lateParticipant and form.hasEnhancedBenefits}">
              <td style="<c:out value="${rowStyle}"/>"><fmt:formatNumber type="currency" value="${curScenario.farmingYear.benefit.enhancedLateEnrolmentPenaltyAfterAppliedBenefitPercent}"/></td>
            </c:if>
