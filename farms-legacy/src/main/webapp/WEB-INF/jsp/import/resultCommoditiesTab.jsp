...
<div id="detailTabs" class="yui-navset"> 
  <ul class="yui-nav"> 
    ...
    <li class="selected"><a href="javascript:showUrl('<c:out value="${commoditiesUrl}"/>')"><em>Commodities (<c:out value="${details.numberOfCommodities}"/>)</em></a></li> 
  ...
  <div class="yui-content"> 
    <c:if test="${details.numberOfCommodities > 0}">
      <div class="searchresults"><div id="commoditiesDatatable"></div></div>
    </c:if>
  </div>
</div>
...
<c:if test="${details.numberOfCommodities > 0}">
    <script type="text/javascript">
       var data = {
          "recordsReturned": <c:out value="${details.numberOfCommodities}"/>,
          "totalRecords": <c:out value="${details.numberOfCommodities}"/>,
          "startIndex":0,
          "sort":null,
          "dir":"asc",
          "pageSize": 10,
          "records":[
            <c:forEach varStatus="loop" var="result" items="${details.commodities}">
              {
                "code":"<c:out value="${result.code}"/>",
                "table":"<c:out value="${result.table}"/>", 
              ...
              }<c:if test="${loop.index < (details.numberOfCommodities-1)}">,</c:if>
            </c:forEach>
          ]
       };
       ...
       YAHOO.util.Event.onDOMReady(function() {
        ...
        var dataTable = new YAHOO.widget.DataTable("commoditiesDatatable", columnDefs, dataSource, configs);
      });
    </script>
</c:if>