...
//  <c:if test="${form.numCodes > 0}">
      ...
      var isMarketCommodityFilter = escape(document.getElementById('isMarketCommodityFilter').value);
      ...
      url = url + "setFilterContext=true" + "&codeFilter=" + codeFilter
						...
						+ "&isMarketCommodityFilter=" + isMarketCommodityFilter;
//  </c:if>
...
<w:ifUserCanPerformAction action="editCodes">
  <script type="text/javascript">
    ...
    function newButtonFunc() {
      goToUrlPreserveContext('newLivestockXref.do');
    }
    //]]>
  </script>
  <u:yuiButton buttonId="newButton" buttonLabel="New.Livestock" function="newButtonFunc"/>
...
//<![CDATA[
        ...
        <c:forEach varStatus="loop" var="result" items="${form.codes}">
          {
            ...
            "marketCommodity":"<c:choose><c:when test="${result.isMarketCommodity}">Y</c:when><c:otherwise>N</c:otherwise></c:choose>"
  ...
  YAHOO.util.Event.onDOMReady(function() {
      Farm.codesColumnDefs = [
         ...
         {key:"marketCommodity", label:"<fmt:message key="Market.Commodity"/>: <select name='isMarketCommodityFilter' id='isMarketCommodityFilter' style='font-size:10px;'><option value=''></option><option value='Y' <c:if test="${form.isMarketCommodityFilter=='Y'}">selected='selected'</c:if>>Y</option><option value='N' <c:if test="${form.isMarketCommodityFilter=='N'}">selected='selected'</c:if> >N</option></select>", sortable:false, className:"tdCenter"}
     ];
    ...
    Farm.filterMarketCommodity = function(value) {
      var marketCommodityFilter = YAHOO.util.Dom.get('isMarketCommodityFilter');
      return Farm.filterSelect(marketCommodityFilter, value);
    };
      ...
      doBeforeCallback : function (req,raw,res,cb) {
        ...
        for (i = 0, l = data.length; i < l; ++i) {
          if(Farm.filterCode(data[i].code)
    	       ...
    	       && Farm.filterMarketCommodity(data[i].marketCommodity)) {
    ...
    Farm.codesDataSource.responseSchema = {
      resultsList: "records",
      fields: ["commodityXrefId","code","description","group","marketCommodity"]
    };
    ...
    Farm.showSelectedRow = function(oArgs) {
      ...
      var farmAction = "farm445";  // edit page
    ...
    YAHOO.util.Event.on('isMarketCommodityFilter','change',Farm.updateFilter);
