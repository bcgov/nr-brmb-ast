...
<h1>
  ...
  <fmt:message key="Livestock"/>
</h1> 
<p></p>

<html:form action="saveLivestockXref" styleId="codeForm" method="post">
  ...
  <fieldset style="width:70%">
    <table class="formInput" style="width:100%">
      ...
      <tr>
        <th style="width:140px"><fmt:message key="Market.Commodity"/>:</th>
        <td>
          <c:choose>
            <c:when test="${canEdit}">
              <html:checkbox property="marketCommodity"/>
            </c:when>
            <c:otherwise>
              <c:choose>
                <c:when test="${result.isMarketCommodity}">
                  <fmt:message key="Yes"/>
                </c:when>
                <c:otherwise>
                  <fmt:message key="No"/>
    ...
    <w:ifUserCanPerformAction action="editCodes">
      ...
      <c:if test="${!form.new}">
        <u:yuiButton buttonId="deleteButton" buttonLabel="Delete" formId="codeForm"
           action="deleteLivestockXref"/>
      </c:if>
      <u:dirtyFormCheck formId="codeForm"/>
    </w:ifUserCanPerformAction>
    <u:yuiButton buttonId="cancelButton" buttonLabel="Cancel" action="farm440"/>
  </div>
