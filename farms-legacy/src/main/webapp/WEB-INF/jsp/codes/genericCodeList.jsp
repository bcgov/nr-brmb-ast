...
//<![CDATA[
  function selectCodeTable() {
    showProcessing();
    document.getElementById("codeTablesForm").submit();
  }
//]]>
</script>

<html:form action="farm265" onsubmit="showProcessing()" method="get" styleId="codeTablesForm">
  <table>
    <tr>
      <td>
			  <table class="searchcriteria">
			    <tr>
			      <th><fmt:message key="Code.Table"/>:</th>
			      <td>
			        <html:select styleId="codeTable" property="codeTable" onchange="selectCodeTable()">
			          <option value=""></option>
			          <html:optionsCollection name="server.list.generic.code.tables"/>
			        </html:select>
			      </td>
			    </tr>
			  </table> 
      ...
      <td>
        <w:ifUserCanPerformAction action="editCodes">
          <c:if test="${ ! empty form.codeTable and form.canCreate}">
            &nbsp;&nbsp;&nbsp;
            <u:yuiButton buttonId="newButton" buttonLabel="New.Code" action="newCode"
...
<c:if test="${ ! empty form.codeTable}">
  ...
  //<![CDATA[
    ...
    YAHOO.util.Event.onDOMReady(function() {
      ...
      Farm.showSelectedRow = function(oArgs) {
        ...
        var farmAction = "farm270";  // edit page
        
        showProcessing();
        document.location.href = farmAction + ".do?codeTable=<c:out value="${form.codeTable}"/>&code=" + code;
      };
