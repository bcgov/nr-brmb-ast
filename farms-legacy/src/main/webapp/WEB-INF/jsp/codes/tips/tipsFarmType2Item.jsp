...
<h1>
  ...
  <fmt:message key="Farm.Type.2"/>
</h1>


<html:form action="saveFarmSubtypeItem" styleId="codeForm" method="post">
          ...
          <c:choose>
            <c:when test="${canEdit}">
              <html:select property="parentName">
                <option value=""></option>
                <html:optionsCollection property="farmTypeListViewItems" />
              </html:select>
            </c:when>
            <c:otherwise>
  ...
  <div style="text-align:right;width:70%">
    <w:ifUserCanPerformAction action="editTipCodes">
      ...
      <c:if test="${!form.new}">
        <u:yuiButton buttonId="deleteButton" buttonLabel="Delete" formId="codeForm" action="deleteFarmSubtypeItem" confirmMessage="delete.farm.subtype.item.warning"/>
      </c:if>
      <u:dirtyFormCheck formId="codeForm"/>
    </w:ifUserCanPerformAction>
    <u:yuiButton buttonId="cancelButton" buttonLabel="Cancel" action="farm490"/>
  </div>
