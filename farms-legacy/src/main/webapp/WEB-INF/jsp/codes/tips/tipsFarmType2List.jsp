...
<span style="float:left;">
  <w:ifUserCanPerformAction action="editTipCodes">
    ...
    <script type="text/javascript">
    //<![CDATA[
      function newFarmSubtype() {
        goToUrlPreserveContext('<html:rewrite action="newFarmSubtypeItem" />');
      }
      var newButton = new YAHOO.widget.Button("newButton", {onclick: {fn: newFarmSubtype}});
    //]]>
    </script>
  ...
  //<![CDATA[
   	...
   	YAHOO.util.Event.onDOMReady(function() {
   	    Farm.codes = {
   	         ...
   	         "pageSize": 10,
   	         "records":[
   	           <c:forEach varStatus="loop" var="result" items="${form.farmSubtypeAItems}">
   	             {
  	               "parent":"<c:out value="${result.parentName}" escapeXml="false" />",
   	               "subtype":"<c:out value="${result.name}" escapeXml="false" />",
           ...
           {key:"subtype", label:"<fmt:message key="Farm.Type.2"/>", sortable:false, className:"tdCenter"}
       ];
	    ...
	    Farm.codesDataSource.responseSchema = {
	      resultsList: "records",
	      fields: ["parent", "subtype", "description", "id"]
	    };
		  
	    Farm.showSelectedRow = function(oArgs) {
          var oRecord = oArgs.record;
          var id = oRecord.getData("id");
          var farmAction = "farm495";  // edit page
          showProcessing();
