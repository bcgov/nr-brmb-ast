...
<h1>
  ...
  <fmt:message key="Farm.Type.3"/>
</h1>


<html:form action="saveFarmTypeItem" styleId="codeForm" method="post">
  ...
  <fieldset style="width:70%">
    <table class="formInput" style="width:100%">
      <tr>
        <th style="width:100px"><fmt:message key="Farm.Type.3"/>:</th>
        <td>
          <c:choose>
            <c:when test="${canEdit}">
              <div style="float:left;width:100%">
                <html:text property="farmTypeName" maxlength="256"/>
              </div>
            </c:when>
            <c:otherwise>
              <html:hidden property="farmTypeName"/>
              <c:out value="${form.farmTypeName}"/>
            </c:otherwise>
          </c:choose>
        </td>
      </tr>     
  ...
  <div id="vueApp">
    ...
    <fieldset>
      ...
      <div v-if="!isDefaultRange" class="infoMessage" v-cloak><fmt:message key="TIP.Using.custom.ranges"/></div>
      <div class="rTable" style="width: 60%">
        ...
        <range-item
          ...
          v-bind:is-default-range="isDefaultRange"
          v-on:remove="farmTypeIncomeData.incomeRange.splice(index, 1)">
        </range-item>
      </div>
      <div v-if="canEdit && !isDefaultRange" v-cloak style="padding: 10px 10px">
  ...
  <div style="text-align:right;width:70%">
    <w:ifUserCanPerformAction action="editTipCodes">
      ...
      <c:if test="${!form.new}">
        <u:yuiButton buttonId="deleteButton" buttonLabel="Delete" formId="codeForm" action="deleteFarmTypeItem" confirmMessage="delete.farm.item.warning"/>
      </c:if>
      <u:dirtyFormCheck formId="codeForm"/>
    </w:ifUserCanPerformAction>
    <u:yuiButton buttonId="cancelButton" buttonLabel="Cancel" action="farm480"/>
  </div>
...
<script>
  ...
  Vue.component('range-item', {
    props: ['item','canEdit', 'isDefaultRange'],
    template: '\
      <div class="rTableRow numeric alignBottom">\
        <div class="rTableCell">\
          <input v-if="canEdit" v-model="item.rangeLow" :disabled="isDefaultRange.length!=0" />\
              <span v-else>{{item.rangeLow}}</span>\
        </div>\
        <div class="rTableCell">\
          <input v-if="canEdit" v-model="item.rangeHigh" :disabled="isDefaultRange.length!=0" />\
          <span v-else>{{item.rangeHigh}}</span>\
        </div>\
        <div class="rTableCell">\
          <input v-if="canEdit" v-model="item.minimumGroupCount" :disabled="isDefaultRange.length!=0" />\
          <span v-else>{{item.minimumGroupCount}}</span>\
        </div>\
        <div class="rTableCell alignBottom">\
          <span class="yui-button yui-push-button">\
            <span class="first-child">\
              <button type="button" v-if="canEdit && !isDefaultRange" v-on:click="$emit(\'remove\')">Remove</button>\
            </span>\
          </span>\
        </div>\
      </div>'
