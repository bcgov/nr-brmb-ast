...
import ca.bc.gov.srm.farm.util.ProducedItemUtils;

public class InventoryImportAction extends InventoryViewAction {
  ...
  protected ActionForward doExecute(final ActionMapping mapping, final ActionForm actionForm, final HttpServletRequest request,
    ...
    logger.debug("Importing Inventory Items...");
    ...
    if (sourceScenarioNumber != null && sourceOperationSchedule != null) {
      ...
      List<ProducedItem> sourceItems = getNonZeroItems(sourceOperation.getProducedItems());
      
      Set<String> sourceInventoryCodes = getInventoryCodeSet(sourceItems);
      logger.debug("Inventory Codes to be imported: " + sourceInventoryCodes);

      FarmingOperation targetOperation = scenario.getFarmingYear().getFarmingOperationBySchedule(sourceOperationSchedule);
      List<ProducedItem> targetItems = getNonZeroItems(targetOperation.getProducedItems());
      ...
      if( ! intersectionInventoryCodes.isEmpty() ) {
        for (Iterator<ProducedItem> itemIter = sourceItems.iterator(); itemIter.hasNext(); ) {
          ProducedItem item = itemIter.next();
        ...
        addMessageWithCodeListIfNotEmpty(messages, inventoryCodesImported, MessageConstants.INFO_INV_IMPORT_ITEMS_IMPORTED);
        addMessageWithCodeListIfNotEmpty(messages, intersectionInventoryCodes, MessageConstants.INFO_INV_IMPORT_ITEMS_ALREADY_EXIST);
      }
      ...
      if(sourceItems.isEmpty()) {
        errors.add("", new ActionMessage(MessageConstants.ERRORS_INV_NO_ITEMS_COULD_BE_IMPORTED));
      }
      ...
      logger.error("Missing importScenarioNumber, unable to import inventory");
      throw new ServiceException("Missing importScenarioNumber, unable to import inventory");
    }
  ...
  private Set<String> getInventoryCodeSet(List<ProducedItem> items) {
    return items.stream()
        .map(i -> i.getInventoryItemCode())
        .collect(Collectors.toSet());
  }

  private List<ProducedItem> getNonZeroItems(List<ProducedItem> producedItems) {
    List<ProducedItem> nonZeroItems = producedItems;
    nonZeroItems = nonZeroItems.stream()
        .filter(p -> ProducedItemUtils.hasNonZeroQuantities(p))
