 ...
 * ListServiceImpl.
 */
final class ListServiceImpl extends BaseService implements ListService {
  ...
  public void setLineItems(Scenario scenario) throws ServiceException {
    ...
    if(scenario.isVerified()) {
      verifiedDate = scenario.getWhenUpdatedTimestamp();
    }
    
    setLineItems(year, verifiedDate);
  }
  ...
  public void setLineItems(Integer year, Date verifiedDate) throws ServiceException {
    ...
    String[] sortOrder = new String[] {"lineItem"};
    listViewArray = (LineItemListView[]) SortUtils.sort(listViewArray, sortOrder);
    
    cacheUserList(LINE_ITEMS, listViewArray);
  }
  ...
  private void createListsFromDatabase() throws ServiceException {
    ...
    try {
      ...
      list = listDao.getScenarioStateCodes(transaction);
      cacheApplicationList(SCENARIO_STATE, list);

      list = listDao.getScenarioCategoryCodes(transaction);
      ...
      list = listDao.getChefSubmssnStatusCodes(transaction);
      cacheApplicationList(CHEF_SUBMSSN_STATUS_CODE, list);

      list = listDao.getVerifiers(transaction);
      cacheApplicationList(VERIFIERS, list);

      String loadListsForScenarioAdjustments = System.getProperty("startup.load.scenario.adj.lists");
      if( ! "N".equals(loadListsForScenarioAdjustments) ){
      
        list = listDao.getInventoryClassCodes(transaction);
        cacheApplicationList(INVENTORY_CLASS, list);
        
        list = listDao.getScenarioClassCodes(transaction);
        ...
        list = listDao.getInventoryGroupCodes(transaction);
        cacheApplicationList(INVENTORY_GROUP, list);
      }
      
      String loadListsForEnrolment = System.getProperty("startup.load.enrolment.lists");
      if( ! "N".equals(loadListsForEnrolment) ){
        
        list = listDao.getRegionalOfficeCodes(transaction);
        cacheApplicationList(REGIONAL_OFFICE, list);
      }
      
      String loadListsForExport = System.getProperty("startup.load.export.lists");
      if( ! "N".equals(loadListsForExport) ) {
        
        list = listDao.getExportClassCodes(transaction);
        cacheApplicationList(EXPORT_CLASS, list);
      }
      
      String loadListsForCodeManagement = System.getProperty("startup.load.code.mgmt.lists");
      if( ! "N".equals(loadListsForCodeManagement) ){
        
        list = listDao.getSectorCodes(transaction);
        cacheApplicationList(SECTOR, list);
        
        list = listDao.getSectorDetailCodes(transaction);
        cacheApplicationList(SECTOR_DETAIL, list);
      }
        
      String loadListsForTipAdmin = System.getProperty("startup.load.tip.admin.lists");
      if( ! "N".equals(loadListsForTipAdmin) ) {
        
        List<LineItemListView> lineItems = listDao.getLineItems(transaction, ProgramYearUtils.getCurrentProgramYear(), null);
        list = new ListView[lineItems.size()];
        list = lineItems.toArray(list);
   ...
   * @param  item  The parameter value.
   */
  private void cacheUserList(final String key, final ListView[] item) {
    CacheFactory.getUserCache().addItem(SERVER_LIST + key, item);
  }
  ...
  private void clearCache() {
    ...
    cache.removeItem(SERVER_LIST + VERIFIERS + EXPIRE_DATE);
  ...
  public void refreshCodeTableList(final String tableName) throws ServiceException {
    ...
    try {
      ...
      if(CodeTables.FARM_TYPE.equals(tableName)) {
        ...
        list = listDao.getTriageQueueCodes(transaction);
        cacheApplicationList(TRIAGE_QUEUE, list);
      } else if(CodeTables.SECTOR.equals(tableName)) {
        list = listDao.getSectorDetailCodes(transaction);
        cacheApplicationList(SECTOR, list);
      } else if(CodeTables.SECTOR_DETAIL.equals(tableName)) {
        list = listDao.getSectorDetailCodes(transaction);
        cacheApplicationList(SECTOR_DETAIL, list);
      } else if(CodeTables.PARTICIPANT_LANGUAGE.equals(tableName)) {
        // ignore since there is currently no cached list for this
      } else if(CodeTables.LINE_ITEMS.equals(tableName)) {
        list = listDao.getSectorDetailCodes(transaction);
        cacheApplicationList(LINE_ITEMS, list);
      } else if(ListServiceConstants.VERIFIERS.equals(tableName)) {
        list = listDao.getVerifiers(transaction);
        cacheApplicationList(VERIFIERS, list);
        Date ExpiryDate = DateUtils.addHours(new Date(), ONE_HOUR_DURATION);
        log.debug("refreshCodeTableList VERIFIERS - ExpiryDate:" + ExpiryDate.toString());
