...
public class CompressionFilter implements Filter {
  ...
  /**
   * List of actions excluded from compression
   */
  ...
  public void init(FilterConfig filterConfig) {
    ...
    if (filterConfig != null) {
      ...
      String exclStr = filterConfig.getInitParameter("exclusions");
      if(exclStr != null) {
        exclusions = new ArrayList<>();
        StringTokenizer st = new StringTokenizer(exclStr, "\n");
        while(st.hasMoreTokens()) {
          String token = st.nextToken();
          if(token != null) {
            String trimmedToken = token.trim();
            if(!trimmedToken.equals("")) {
              exclusions.add(trimmedToken);
            }
          }
        }
      }
  ...
  throws IOException, ServletException {
    ...
    if (!supportCompression || excluded) {
      if (debug > 0) {
   ...
   * @return true if the path ends with one of the excluded actions
   */
  private boolean isExcluded(ServletRequest request) {
    boolean excluded = false;
    String path = ((HttpServletRequest) request).getServletPath();
    if(exclusions != null && path != null) {
      for(String excl : exclusions) {
        if(excl != null && path.endsWith(excl.trim())) {
          excluded = true;
          break;
        }
      }
  ...
  /**
   * Set filter config This function is equivalent to init. Required by Weblogic
   * 6.1
   * 
   * @param filterConfig
   *          The filter configuration object
