...
- Local dev: run via the IntelliJ SmartTomcat plugin, not a Maven plugin. Its generated Tomcat instance (`catalina.base`) lives *outside* the repo, under `~/.SmartTomcat/nr-brmb-ast/farms-legacy/` — that's where `conf/` (including the local `jdbc/farms_rest` datasource) and `logs/` are. The plugin mounts only `target/classes` as `WEB-INF/classes`, so test resources are never visible to a locally running app.
