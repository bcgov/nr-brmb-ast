...
- Local dev: run via the IntelliJ SmartTomcat plugin, not a Maven plugin. Two directories are involved and **both matter**: `../.smarttomcat/farms-legacy/conf/` (repo root, gitignored) is the *source* conf the plugin seeds from — it holds the hand-added `jdbc/farms_rest` JNDI datasource — and `~/.SmartTomcat/nr-brmb-ast/farms-legacy/` is the generated `catalina.base` it seeds *into*, where `logs/` and the running instance live. Deleting the former makes the plugin re-seed from the stock Tomcat install, silently dropping the datasource and failing startup with `NameNotFoundException: Name [jdbc/farms_rest] is not bound`. The plugin mounts only `target/classes` as `WEB-INF/classes`, so test resources are never visible to a locally running app.
