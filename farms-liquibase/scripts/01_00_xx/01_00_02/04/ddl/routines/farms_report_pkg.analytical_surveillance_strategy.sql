create or replace function farms_report_pkg.analytical_surveillance_strategy(
    in in_program_year farms.program_year.year%type
)
returns refcursor
language plpgsql
as $$
    ...
    open cur for
        ...
        with scenarios as (
            select m.participant_pin,
                   ...
                   (bct.total_allowable_income + bct.accrual_adjustments_crop_inventory + bct.accrual_adjustments_livestock_inventory + bct.accrual_adjustments_receivables) accrued_allowable_revenue,
                   bct.expense_accrual_adjustments accrued_allowable_expenses,
                   cl.program_year_margin margin,
                   cl.production_insurance_deemed_benefit,
                   cl.adjusted_reference_margin reference_margin_before_limit,
                   cl.reference_margin_limit,
                   bct.accrual_adjustments_purchased_inputs,
                   bct.accrual_adjustments_payables,
            ...
            join farms.program_year py on py.program_year_id = m.program_year_id
            join farms.agristability_scenario s on s.agristability_scenario_id = m.agristability_scenario_id
            join farms.benefit_calculation_total bct on bct.agristability_scenario_id = m.agristability_scenario_id
            join farms.agristability_claim cl on cl.agristability_scenario_id = m.agristability_scenario_id
            join farms.sector_detail_code sdc on sdc.sector_detail_code = farms_report_pkg.get_sector_detail_code(s.agristability_scenario_id)
            join farms.sector_detail_xref sdx on sdx.sector_detail_code = sdc.sector_detail_code
            join farms.sector_code sec on sec.sector_code = sdx.sector_code
            where m.year = in_program_year
            and py.non_participant_indicator = 'N'
            ...
            select s.*,
                   sum(bct.production_margin_accrual_adjustments) over (partition by s.agristability_scenario_id) sum_production_margin_accrual_adjustments,
                   min(bct.production_margin_accrual_adjustments) over (partition by s.agristability_scenario_id) min_production_margin_accrual_adjustments,
                   max(bct.production_margin_accrual_adjustments) over (partition by s.agristability_scenario_id) max_production_margin_accrual_adjustments,
                   first_value(bct.production_margin_accrual_adjustments) over (partition by s.agristability_scenario_id order by py.year) first_production_margin_accrual_adjustments,
                   count(*) over (partition by s.agristability_scenario_id) ref_year_count,
                   count(case when rs.used_in_calculation_indicator = 'Y' and bct.production_margin_after_structure_changes < 0 then 1 end) over (partition by s.agristability_scenario_id) negative_margin_used_in_calculation,
                   row_number() over (partition by s.agristability_scenario_id order by py.year) s2_row_number
            from scenarios s
            join farms.reference_scenario rs on rs.for_agristability_scenario_id = s.agristability_scenario_id
            join farms.agristability_scenario rsas on rsas.agristability_scenario_id = rs.agristability_scenario_id
            join farms.program_year_version pyv on pyv.program_year_version_id = rsas.program_year_version_id
            join farms.program_year py on py.program_year_id = pyv.program_year_id
            join farms.benefit_calculation_total bct on bct.agristability_scenario_id = rs.agristability_scenario_id
            ...
            select s.agristability_scenario_id,
                   sum(case
                       when li.eligibility_indicator = 'N' and li.line_item in (9760,9808,9821,9807,9825,9798,9824,9796,9823,9809,9826,9795,9804,9820,9819,9792) then rie.amount * fo.partnership_percent
                       else 0
                   end) overhead_expenses,
                   sum(case
                       when li.eligibility_indicator = 'N' and li.line_item in (9805,9811,9810,9765,9829) then rie.amount * fo.partnership_percent
                       else 0
                   end) finance_and_capital_expenses,
                   sum(case
                       when li.eligibility_indicator = 'N' and li.line_item in (9936) then rie.amount * fo.partnership_percent
                       else 0
                   end) depreciation,
                   sum(case
                       when li.eligibility_indicator = 'N' and li.line_item in (9816) then rie.amount * fo.partnership_percent
                       else 0
                   end) non_arms_length_salaries
            from scenarios3 s
            join farms.farming_operation fo on fo.program_year_version_id = s.program_year_version_id
            left outer join farms.reported_income_expenses rie on rie.farming_operation_id = fo.farming_operation_id
                                                                and (s.agristability_scenario_id = rie.agristability_scenario_id or rie.agristability_scenario_id is null)
                                                                and rie.expense_indicator = 'Y'
            left outer join farms.line_item li on li.line_item = rie.line_item
            ...
            select s.participant_pin,
                   ...
                   s.margin,
                   coalesce(s.production_insurance_deemed_benefit, 0) production_insurance_deemed_benefit,
                   e.overhead_expenses,
            ...
            select s.participant_pin,
                   ...
                   coalesce(s.production_insurance_deemed_benefit * s.combined_farm_percent, 0) production_insurance_deemed_benefit,
        ...
        select s.participant_pin,
               ...
               round(s.production_insurance_deemed_benefit, 2) production_insurance_deemed_benefit,
               ...
               s.viability_test_failure,
               s.combined_farm_percent,
               s.late_participant_indicator -- added for Analytical report
