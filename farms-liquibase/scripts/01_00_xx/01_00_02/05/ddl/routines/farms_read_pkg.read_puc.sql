    ...
    open cur for
        with a as (
            ...
            select p.farming_operation_id,
                   ...
                   sum(ri.on_farm_acres) over (partition by p.cra_productve_unit_capacity_id, p.adj_productive_unit_capacity_id) cra_on_farm_acres,
                   sum(ri.unseedable_acres) over (partition by p.cra_productve_unit_capacity_id, p.adj_productive_unit_capacity_id) cra_unseedable_acres,
                   row_number() over (partition by p.cra_productve_unit_capacity_id, p.adj_productive_unit_capacity_id order by ri.reported_inventory_id) row_num
            from p
