create or replace function farms_calculator_pkg.read_operations_for_puc_import(
...
begin
    ...
    select sv.program_year_version_number,
           ...
           select *
               ...
               where puc.participnt_data_src_code = 'CRA'
               and puc.farming_operation_id = fo.farming_operation_id
               and (coalesce(puc.agristability_scenario_id::text, '') = '' or (sv.scenario_class_code in ('USER','REF') and puc.agristability_scenario_id = sv.agristability_scenario_id))
           )
           then 'Y' else 'N' end has_cra_productive_units,
           case when exists (
               select *
               from farms.farm_productve_unit_capacities puc
               where puc.participnt_data_src_code = 'LOCAL'
               and puc.farming_operation_id = fo.farming_operation_id
               and (coalesce(puc.agristability_scenario_id::text, '') = '' or (sv.scenario_class_code in ('USER','REF') and puc.agristability_scenario_id = sv.agristability_scenario_id))
            )
            then 'Y' else 'N' end has_local_productive_units
    ...
    and exists (
        select *
        from farms.farm_productve_unit_capacities puc
        where puc.farming_operation_id = fo.farming_operation_id
        and (coalesce(puc.agristability_scenario_id::text, '') = '' or (sv.scenario_class_code in ('USER','REF') and puc.agristability_scenario_id = sv.agristability_scenario_id))
    )
