create or replace function farms_export_pkg.f30_ade(
...
begin
    ...
    open cur for
        ...
        select max(ac.participant_pin) participant_pin,
               ...
               max(clm.unadjusted_reference_margin) unadjusted_reference_margin,
               max(clm.adjusted_reference_margin) adjusted_reference_margin,
        ...
        join farms.farm_agristability_claims clm on clm.agristability_scenario_id = t.agristability_scenario_id
        left outer join farms.farm_whole_farm_participants whole on whole.program_year_version_id = t.program_year_version_id
        where py.non_participant_ind = 'N'
        group by clm.agristability_claim_id
