...
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.insurable_value IS E'INSURABLE VALUE is the dollar amount of insurance per unit that the AgriStability program requires the participant to carry.';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.inventory_item_code IS E'INVENTORY ITEM CODE is a unique code for the object INVENTORY ITEM CODE described as a numeric code used to uniquely identify an inventory item. Examples of codes and descriptions are 2 - Canadian Wheat Board Payments, 3 - Barley (seed), 4 - Beans (Dry Edible).';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.line_number IS E'LINE NUMBER is the number of a line in a CSV file. Everytime an IVPR CSV file is loaded, all the IVPR staging data will be deleted. Also used for generating errors to the user about data problems in the input file.';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.premium_rate IS E'PREMIUM RATE is the main portion of the premium the participant pays for insurance of their production it is the percentage of the total dollar value they have insured for this INVENTORY ITEM CODE.';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.program_year IS E'PROGRAM YEAR is the PROGRAM YEAR this data pertains to.';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.revision_count IS E'REVISION COUNT is a counter identifying the number of times this record as been modified. Used in the web page access to determine if the record as been modified since the data was first retrieved.';
COMMENT ON COLUMN farms.farm_zivpr_iv_premium_rates.when_created IS E'WHEN CREATED indicates when the physical record was created in the database.';
