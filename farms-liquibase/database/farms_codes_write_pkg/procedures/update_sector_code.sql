create or replace procedure farms_codes_write_pkg.update_sector_code(
   ...
   in in_description farms.farm_sector_codes.description%type,
   in in_expiry_date farms.farm_sector_codes.expiry_date%type,
   in in_revision_count farms.farm_sector_codes.revision_count%type,
   in in_user farms.farm_sector_codes.who_updated%type
)
language plpgsql
as $$
begin
