create or replace procedure farms_tip_admin_pkg.expenses_benchmarks(
...
begin
    ...
    using (
      ...
      (
        ...
        join farms.farm_tip_benchmark_years tby on (   (tby.tip_benchmark_year_id = parent_trr.tip_benchmark_year_id_type_3 and parent_trr.farm_type_3_threshold_met_ind = 'Y')
                                              or (tby.tip_benchmark_year_id = parent_trr.tip_benchmark_year_id_type_2 and parent_trr.farm_type_2_threshold_met_ind = 'Y')
                                              or (tby.tip_benchmark_year_id = parent_trr.tip_benchmark_year_id_type_1 and parent_trr.farm_type_1_threshold_met_ind = 'Y'))
        left outer join farms.farm_tip_report_expenses tre on tre.tip_report_result_id = parent_trr.tip_report_result_id
        where coalesce(tby.parent_tip_benchmark_year_id::text, '') = ''
