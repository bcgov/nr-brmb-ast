create or replace procedure farms_tip_admin_pkg.program_year_report_results(
...
begin
    ...
    with op as (
      select pyv.program_year_version_id,
             ...
             case when fo.partnership_pin = 0 then null else row_number() over (partition by fo.partnership_pin order by pyv.when_created desc) end partnership_row_num
    ...
    i1 as (
      select fo.farming_operation_id,
             ft3.farm_type_3_name,
             ft2.farm_type_2_name,
             ft1.farm_type_1_name,
             case when coalesce(op.partnership_row_num::text, '') = '' or partnership_row_num = 1 then 'Y' else 'N' end use_for_benchmarks_ind,
             -- income
    ...
    i2 as(
      select farming_operation_id,
             ...
             row_number() over (partition by farming_operation_id, farm_type_1_name order by 1) i2_row_num
      from i1
    ),
    i3 as(
      select i2.*,
             ...
             case when commodity_income = 0 then 0 else round((farm_type_3_income / commodity_income)::numeric, 4) end farm_type_3_percent,
             case when commodity_income = 0 then 0 else round((farm_type_2_income / commodity_income)::numeric, 4) end farm_type_2_percent,
             case when commodity_income = 0 then 0 else round((farm_type_1_income / commodity_income)::numeric, 4) end farm_type_1_percent
      from i2
      where i2_row_num = 1
         and (farm_type_1_name is not null and farm_type_1_name::text <> '') 
    ),
    i4 as(
      select i3.*,
             case when farm_type_3_percent > 0    then 'Y' else 'N' end farm_type_3_threshold_met_ind,
             case when farm_type_2_percent > 0.50 then 'Y' else 'N' end farm_type_2_threshold_met_ind,
             case when farm_type_1_percent > 0.50 then 'Y' else 'N' end farm_type_1_threshold_met_ind,
             row_number() over (partition by farming_operation_id order by farm_type_3_percent desc, farm_type_2_percent desc, farm_type_1_percent desc, farm_type_1_name) farm_type_rank
      from i3
    ),
    i5 as (
      select i4.*
      from i4
      where farm_type_rank = 1
        and farm_type_3_threshold_met_ind = 'Y'
    )
    select ac.participant_pin,
           ...
           farm_type_2_percent,
           farm_type_1_percent,
           farm_type_3_threshold_met_ind,
           farm_type_2_threshold_met_ind,
           farm_type_1_threshold_met_ind,
           total_income,
           total_expenses,
           allowable_income,
           allowable_expenses,
    ...
    from i5
    join farms.farm_farming_operations fo on i5.farming_operation_id = fo.farming_operation_id
    join farms.farm_program_year_versions pyv on pyv.program_year_version_id = fo.program_year_version_id
    join farms.farm_program_years py on py.program_year_id = pyv.program_year_id
    join farms.farm_agristability_clients ac on ac.agristability_client_id = py.agristability_client_id 
    ) n
    on (o.program_year_id = n.program_year_id and o.alignment_key = n.alignment_key and coalesce(o.parent_tip_report_result_id::text, '') = '')
    when not matched then
      ...
      values (nextval('farms.farm_trr_seq'),
       ...
       n.farm_type_2_name,
       n.farm_type_1_name,
       n.farm_type_3_percent,
       n.farm_type_2_percent,
       n.farm_type_1_percent,
       n.farm_type_3_threshold_met_ind,
       n.farm_type_2_threshold_met_ind,
       n.farm_type_1_threshold_met_ind,
       n.total_income,
       n.total_expenses,
       n.allowable_income,
