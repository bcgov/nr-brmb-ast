...
**An in-place edit is never applied by `liquibase update` in an environment where its changeset already ran.** Merging and deploying the image does not update the routine, and clearing checksums does not make the changeset run again. After merging a routine/DDL change, it must be applied in each environment (dev, test, prod) separately — either run the file's `CREATE OR REPLACE` directly against that database, or delete the changeset's row from `DATABASECHANGELOG` and re-run the migration Job. A changeset groups a whole package's functions (or procedures), e.g. `01_00_00/00/farms_enrolment_read_pkg/functions`, so the second option re-applies every routine in that group — harmless for `CREATE OR REPLACE`, but check before doing it for anything that isn't idempotent.
